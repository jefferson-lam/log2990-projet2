...
describe('ResizeHeightService', () => {
    ...
    it('resize should set style.top and height of previewSelectionCanvas to appropriate values if cursor under initial top', () => {
        ...
        service.resize(moveEvent);
        expect(service.selectionComponent.previewSelectionCanvas.style.top).toBe(service.selectionComponent.initialPosition.y + 'px');
    ...
    it('resizeSquare should not set style.top if combined and style.top not over initial top', () => {
        service.selectionComponent.previewSelectionCanvas.style.top = service.selectionComponent.initialPosition.y + 1 + 'px';
        const difference = CanvasConstants.DEFAULT_HEIGHT - CanvasConstants.MIN_WIDTH_CANVAS;
        const expectedTop = parseInt(service.selectionComponent.previewSelectionCanvas.style.top, 10) + difference + 'px';
        service.resizeSquare(true);
