...
describe('ResizeWidthService', () => {
    ...
    it('resize should set style.left and width of previewSelectionCanvas to appropriate values if cursor over initial left', () => {
        service.selectionComponent.initialPosition.x = pointerPosition - 1;
        service.resize(moveEvent);
        expect(service.selectionComponent.previewSelectionCanvas.style.left).toBe(service.selectionComponent.initialPosition.x + 'px');
        expect(service.selectionComponent.previewSelectionCanvas.width).toBe(pointerPosition - service.selectionComponent.initialPosition.x);
    });

    it('resize should set style.left and width of previewSelectionCanvas to appropriate values if cursor under initial left', () => {
        service.selectionComponent.initialPosition.x = pointerPosition + 1;
        service.resize(moveEvent);
        expect(service.selectionComponent.previewSelectionCanvas.style.left).toBe(pointerPosition + 'px');
        expect(service.selectionComponent.previewSelectionCanvas.width).toBe(service.selectionComponent.initialPosition.x - pointerPosition);
    });
    ...
    it('resizeSquare should set style.left if combined and style.left under initial left', () => {
    ...
    it('resizeSquare should not set style.left if not combined and style.left under initial left', () => {
        service.selectionComponent.previewSelectionCanvas.style.left = service.selectionComponent.initialPosition.x - 1 + 'px';
        const difference = CanvasConstants.DEFAULT_WIDTH - CanvasConstants.MIN_HEIGHT_CANVAS;
    ...
    it('resizeSquare should not set style.left if combined and style.left not under initial leftt', () => {
        service.selectionComponent.previewSelectionCanvas.style.left = service.selectionComponent.initialPosition.x + 1 + 'px';
        const difference = CanvasConstants.DEFAULT_WIDTH - CanvasConstants.MIN_HEIGHT_CANVAS;
        const expectedTop = parseInt(service.selectionComponent.previewSelectionCanvas.style.left, 10) + difference + 'px';
        service.resizeSquare(true);
